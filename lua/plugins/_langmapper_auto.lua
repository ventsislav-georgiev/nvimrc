-- ref: https://github.com/Wansmer/langmapper.nvim/blob/main/lua/langmapper/auto.lua
-- forced which_key_ignore
  ...
  for _, mode in ipairs(allowed_modes) do
    ...
    for _, map in ipairs(maps) do
      if not u.lhs_forbidden(map.lhs) then
        local lhs, modes = map.lhs,handle_modes(map.mode, allowed_modes)
        table.insert(mappings, { lhs = lhs, desc = 'which_key_ignore', mode = modes })
      end
    end
  end
  return mappings
